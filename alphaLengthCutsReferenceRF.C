...
void alphaLengthCutsReferenceRF(){

  using namespace RooFit;
  using namespace RooStats;


  string foils="/Users/fang/SuperNEMO/Analysis/RadonBKG/SimuRecData/0Gauss_1E6/NoGammaTracking/sensitivityBulk.root";
  string surface="/Users/fang/SuperNEMO/Analysis/RadonBKG/SimuRecData/0Gauss_1E6/NoGammaTracking/sensitivitySurf.root";
  string wires="/Users/fang/SuperNEMO/Analysis/RadonBKG/SimuRecData/0Gauss_1E6/NoGammaTracking/sensitivityWire.root";
  ...
  int total_number_entries=0;
    ...
    /*
     double activity_tracker=41.3E-3; // 41.3 ± 4.7 mBq 7.8% on surf and 92.2% on wires // old number 2.28E-3 with flushing
     double activity_bulk=4.94E-3;     //old  nembers possibly wrong 15.4E-3;//70E-6; //70 µBq target, 10 µBq/kg
     double activity_surf=0.34E-3;     //old number 0.1685E-3;//activity_tracker*0.078; //0.178 mBq //3.37mBq/20
     //  double activity_wire=42.3E-3;//activity_tracker*0.922; // 2.10 mBq
     double activity_wire=3.92E-3; // old 2.1E-3;// 41.3 ± 4.7 mBq  92.2% on wires give 38.08 and devided by 9.71 factor//activity_tracker*0.922; // 2.10 mBq
     */
    double activity_tracker=41.3E-3;
    double activity_bulk=4.94E-3;
    double activity_surf=0.85E-3;
    double activity_wire=3.40E-3;
...
//  fitted_activity_bulk->setVal(activity_bulk*(exposure_in_seconds*efficiency_bulk));
...
//  fitted_activity_surf->setVal(activity_surf*(exposure_in_seconds*efficiency_surf));
  ...
  //fitted_activity_wire->setVal(activity_wire*(exposure_in_seconds*efficiency_wire));  //no need to set a value for fitted_activity_wire coz roofit normalistion fit will giv a value
...
//  RooHistPdf pdf("pdf","pdf",track_length_obs,datahist);
...
/*
  pdf_bulk = new RooHistPdf("pdf_bulk","pdf_bulk",track_length_obs, *datahist_bulk);
  pdf_surf = new RooHistPdf("pdf_surf","pdf_surf",track_length_obs, *datahist_surf);
  pdf_wire = new RooHistPdf("pdf_wire","pdf_wire",track_length_obs, *datahist_wire);
*/
...
//  RooAddPdf model("model","model",RooArgSet(*pdf_wire),RooArgList(*fitted_activity_wire));
  ...
  //RooRandom::randomGenerator()->SetSeed(i_pseudo);
...
/*
    cout << activity_bulk <<" "<< fitted_activity_bulk->getValV() << endl;
    cout << activity_surf <<" "<< fitted_activity_surf->getValV() << endl;
    cout << activity_wire <<" "<< fitted_activity_wire->getValV() << endl;


    cout << activity_bulk*(exposure_in_seconds*efficiency_bulk) <<" "<< fitted_activity_bulk->getValV() << endl;
    cout << activity_surf*(exposure_in_seconds*efficiency_surf) <<" "<< fitted_activity_surf->getValV() << endl;
    cout << activity_wire*(exposure_in_seconds*efficiency_wire) <<" "<< fitted_activity_wire->getValV() << endl;
  */
...
h_fitted_activity_wire->SetTitle("h_fitted_activity_wire");

    TF1 *myfit = new TF1("myfit","gaus");

//    h_fitted_activity_wire->Fit(myfit,"","",
  //         h_fitted_activity_wire->GetBinCenter(std::max((int)(n_pseudo)/200,1)),
  //         h_fitted_activity_wire->GetBinCenter(h_fitted_activity_wire->GetNbinsX()+1));

   h_fitted_activity_wire->Fit(myfit,"","",fit_limit_wire[0],fit_limit_wire[1]); // option "LE" give succeddful fit
//   h_fitted_activity_wire->Fit(myfit,"","",0.0*activity_wire, 2.0*activity_wire); // option "LE" give succeddful fit
    ...
    cout << exposure <<"     "<<myfit->GetParameter(2) <<"       "<< myfit->GetParameter(1) << endl;
